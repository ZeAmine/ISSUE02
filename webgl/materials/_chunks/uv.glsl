...
vec2 scaleUV(vec2 uv,vec2 scale){
    return scaleUV(uv,scale,vec2(.5));
}

// Scale 1
vec2 scaleTexture(vec2 uv,vec2 factor){
    vec2 scale=2.-factor;
    return vec2(uv.x*scale.x+(1.-scale.x)*.5,uv.y*scale.y+(1.-scale.y)*.5);
}
